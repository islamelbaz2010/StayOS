...
export default async function SupportPage({
  ...
  const page = await fetchPublicPage("support", locale);
  const phone = process.env.NEXT_PUBLIC_SUPPORT_WHATSAPP_NUMBER?.trim() || "";
        ...
        <div className="mt-8 rounded-xl bg-white p-6 shadow-card">
          ...
          {phone ? (
            <a
              ...
              className="mt-4 flex items-center justify-center gap-2 rounded-lg bg-success-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-success-700"
            >
            ...
            <div className="mt-4 rounded-lg bg-neutral-50 p-4 text-center">
              <p className="text-sm text-neutral-600">{t("notConfigured")}</p>
            </div>
          )}
