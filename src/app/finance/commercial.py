"""Canonical StayOS commercial engine — the single source of truth for
booking economics.

FOUNDER DECISION (final commercial model):

- StayOS economics total 12% of the accommodation amount.
- Internally allocated 6% host-side + 6% guest-side — this split is a
  reporting/ledger concept only. It is NEVER shown to the guest.
- The Guest-facing price is strictly all-inclusive: the amount shown in
  search, listing, booking, checkout, confirmation, and trips is the final
  price ("Includes VAT"). No service-fee line, no commission or payout
  disclosure.
- Fee base: the 12% applies to the accommodation amount only — never to
  cleaning/additional fees, pass-through charges, taxes, or deposits.
- Host payout derives from the SAME computation the guest was charged
  with: ``host_net = taxable_amount - platform_share``.

FOUNDER DECISION (VAT — separate tax, supersedes the carve-out model):

- VAT at ``VAT_RATE_PCT`` (14%) is a SEPARATE tax — it is NOT part of the
  12% StayOS commercial share and is never carved out of it.
- Tax base: the taxable booking amount = accommodation (after applicable
  discounts) + cleaning. VAT is never applied to the platform share, the
  host payout, VAT itself, or operational metadata.
- VAT is added ON TOP of the taxable amount: ``guest_total = taxable +
  vat``. The guest sees it as an explicit "VAT 14%" line.
- VAT is a liability owed to the tax authority (``VAT_PAYABLE``), not
  platform revenue and not host earnings. It participates in refunds
  proportionally to the refunded taxable amount.
- The closed-alpha share waiver does NOT waive VAT — a waived booking
  still carries VAT on its taxable amount.

All money is integer EGP minor-major units; never floats.
"""

from dataclasses import dataclass

from app.config import settings


@dataclass(frozen=True)
class BookingEconomics:
    """Internal economics of one booking. Guest surfaces must only ever
    render ``guest_total_egp`` plus the ``vat_egp`` tax line — every other
    field is internal."""

    accommodation_egp: int  # nightly prices after the applicable discount
    cleaning_fee_egp: int
    taxable_amount_egp: int  # accommodation + cleaning — the VAT tax base
    vat_egp: int  # separate tax on the taxable amount — owed to VAT_PAYABLE
    guest_total_egp: int  # taxable + VAT — the final price the guest pays
    platform_share_egp: int  # StayOS economics (12% of accommodation)
    host_net_egp: int  # what the host is owed (taxable - platform share)
    # Internal 6% + 6% allocation — ledger/reporting only.
    host_side_share_egp: int
    guest_side_share_egp: int


def compute_vat(taxable_amount_egp: int) -> int:
    """VAT on a taxable booking amount at the configured rate.

    VAT is a separate tax — it never depends on the platform share or the
    alpha waiver. Returns 0 for non-positive bases.
    """
    if taxable_amount_egp <= 0:
        return 0
    return int(round(taxable_amount_egp * settings.VAT_RATE_PCT))


def vat_inclusive_portion(amount_egp: int) -> int:
    """VAT component of an amount that already includes VAT — used for
    refund splits and VAT-inclusive custom offers. ``portion + taxable``
    always reconstructs the original amount exactly."""
    if amount_egp <= 0:
        return 0
    return amount_egp - int(round(amount_egp / (1 + settings.VAT_RATE_PCT)))


def compute_booking_economics(
    accommodation_egp: int,
    cleaning_fee_egp: int = 0,
    *,
    platform_share_waived: bool = False,
) -> BookingEconomics:
    """Split a booking into VAT, platform share and host net.

    ``platform_share_waived`` implements the closed-alpha incentive
    (first ALPHA_HOST_FREE_BOOKINGS completed bookings carry no platform
    share) — it affects ONLY the StayOS commercial share. VAT is a
    separate tax on the taxable amount and is never waived by it.
    """
    taxable = accommodation_egp + cleaning_fee_egp
    vat = compute_vat(taxable)
    guest_total = taxable + vat
    if platform_share_waived:
        platform_share = 0
    else:
        platform_share = int(
            round(accommodation_egp * settings.PLATFORM_TOTAL_SHARE_PCT)
        )
    host_net = taxable - platform_share
    # Split the platform share into the two internal 6% allocations,
    # putting any rounding remainder on the host-side bucket so the two
    # always sum exactly to platform_share.
    guest_side = int(
        round(accommodation_egp * settings.GUEST_SIDE_SHARE_PCT)
    ) if platform_share else 0
    host_side = platform_share - guest_side
    return BookingEconomics(
        accommodation_egp=accommodation_egp,
        cleaning_fee_egp=cleaning_fee_egp,
        taxable_amount_egp=taxable,
        vat_egp=vat,
        guest_total_egp=guest_total,
        platform_share_egp=platform_share,
        host_net_egp=host_net,
        host_side_share_egp=host_side,
        guest_side_share_egp=guest_side,
    )


def guest_all_in_price_for_host_target(host_target_net_egp: int) -> int:
    """Gross-up helper for the host earnings simulator: if a host thinks
    in terms of a target net amount, the equivalent all-inclusive guest
    price is ``target / (1 - 0.12)`` plus VAT on the taxable amount.
    Rounds to the nearest EGP such that the resulting host net is never
    below the target."""
    rate = settings.PLATFORM_TOTAL_SHARE_PCT
    if rate >= 1.0:
        raise ValueError("Platform share must be below 100%")
    taxable = int(round(host_target_net_egp / (1.0 - rate)))
    return taxable + compute_vat(taxable)
