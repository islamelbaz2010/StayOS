...
from . import services as operations_services

CONSUMER_NAME = "operations"
...
async def process_outbox_event(
    ...
    if event.event_type == "booking.checked_out":
        await operations_services.handle_checkout_event(session, payload)
    elif event.event_type == "booking.checked_in":
        await operations_services.handle_checkin_event(session, payload)
    elif event.event_type == "booking.cancelled":
        await operations_services.handle_cancel_event(session, payload)
